...
const incomeSummary = computed(() => statisticsStore.getIncomeExpenseSummary(startDate.value, endDate.value))
const topExpenses = computed(() => statisticsStore.getCategoryExpenses(startDate.value, endDate.value, 5))
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div class="card bg-base-100 shadow-md">
        <div class="card-body">
          ...
          <p class="text-2xl font-bold">{{ formatCurrency(accountStore.totalBalance) }}</p>
          <div class="card-actions justify-end mt-2">
            <button class="btn btn-sm btn-ghost" @click="navigateToAccounts">
              Details <span class="iconify ml-1" data-icon="mdi:chevron-right"></span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="card-body">
          ...
          <div class="grid grid-cols-2 gap-2 mt-2">
            <div>
              ...
              <p class="text-lg font-semibold text-success">{{ formatCurrency(incomeSummary.income) }}</p>
            </div>
            <div>
              <p class="text-sm">Ausgaben</p>
              <p class="text-lg font-semibold text-error">{{ formatCurrency(incomeSummary.expense) }}</p>
            </div>
          </div>
          <div class="card-actions justify-end mt-2">
      ...
      <div class="card bg-base-100 shadow-md">
        <div class="card-body">
          ...
          <p v-if="false" class="text-lg font-semibold">{{ formatCurrency(0) }}</p>
        ...
        <div class="card bg-base-100 shadow-md">
          <div class="card-body">
            <div class="flex justify-between items-center mb-4">
              ...
              <button class="btn btn-sm btn-ghost" @click="navigateToTransactions">
                Alle anzeigen <span class="iconify ml-1" data-icon="mdi:chevron-right"></span>
              </button>
            </div>
                ...
                <tbody>
                  <tr v-for="tx in recentTransactions" :key="tx.id">
                    ...
                    <td>
                      ...
                      <span v-else class="text-opacity-60">Keine Kategorie</span>
                    </td>
                    <td :class="tx.amount >= 0 ? 'text-success text-right' : 'text-error text-right'">
                      {{ formatCurrency(tx.amount) }}
                    </td>
                  </tr>
                  <tr v-if="recentTransactions.length === 0">
                    <td colspan="4" class="text-center py-4">Keine Transaktionen vorhanden</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div class="card bg-base-100 shadow-md">
          <div class="card-body">
              ...
              <table class="table table-zebra w-full">
                ...
                <tbody>
                  <tr v-for="(month, index) in monthlyTrend" :key="index">
                    <td>{{ month.month }}</td>
                    <td class="text-success text-right">{{ formatCurrency(month.income) }}</td>
                    <td class="text-error text-right">{{ formatCurrency(month.expense) }}</td>
                    <td :class="month.income - month.expense >= 0 ? 'text-success text-right' : 'text-error text-right'">
                      {{ formatCurrency(month.income - month.expense) }}
                    </td>
                  </tr>
                  <tr v-if="monthlyTrend.length === 0">
                    <td colspan="4" class="text-center py-4">Keine Daten vorhanden</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div class="card-actions justify-end mt-4">
              <button class="btn btn-sm btn-ghost" @click="navigateToStatistics">
                Mehr Statistiken <span class="iconify ml-1" data-icon="mdi:chevron-right"></span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-md">
          <div class="card-body">
            ...
            <div v-if="topExpenses.length > 0">
              <div v-for="expense in topExpenses" :key="expense.categoryId" class="mb-3">
                <div class="flex justify-between items-center mb-1">
                  <span>{{ expense.name }}</span>
                  <span class="text-error">{{ formatCurrency(expense.amount) }}</span>
                </div>
                <progress class="progress progress-error w-full" :value="expense.amount" :max="topExpenses[0].amount"></progress>
              </div>
            </div>
            ...
            <div class="card-actions justify-end mt-2">
              <button class="btn btn-sm btn-ghost" @click="navigateToBudgets">
                Budgets verwalten <span class="iconify ml-1" data-icon="mdi:chevron-right"></span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="card-body">
            ...
            <div v-if="savingsGoals.length > 0">
              <div v-for="goal in savingsGoals" :key="goal.id" class="mb-4">
                ...
                <progress class="progress progress-accent w-full" :value="goal.progress" max="100"></progress>
