...
const importData = (event: Event) => {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!data.accounts || !data.accountGroups || !data.categories || 
          !data.categoryGroups || !data.transactions || !data.planningTransactions || 
          !data.tags) {
      ...
      if (confirm('Möchten Sie wirklich alle Daten importieren? Bestehende Daten werden überschrieben.')) {
        ...
        localStorage.setItem('finwise_account_groups', JSON.stringify(data.accountGroups))
        localStorage.setItem('finwise_categories', JSON.stringify(data.categories))
        localStorage.setItem('finwise_category_groups', JSON.stringify(data.categoryGroups))
        localStorage.setItem('finwise_transactions', JSON.stringify(data.transactions))
        localStorage.setItem('finwise_planning_transactions', JSON.stringify(data.planningTransactions))
  ...
  <div>
    ...
    <div class="card bg-base-100 shadow-md mt-6">
      <div class="card-body">
        ...
        <p>FinWise ist eine persönliche Finanzverwaltungsanwendung, die Ihnen hilft, Ihre Finanzen zu verwalten und zu planen.</p>
        
        <div class="mt-4">
          <p class="text-sm text-base-content/70">Version: 0.1.0</p>
          <p class="text-sm text-base-content/70">© {{ new Date().getFullYear() }} - FinWise</p>
        </div>
      </div>
    </div>
  </div>
