### Änderungen gegenüber letzter Variante:
- Hinzugefügt: Beschreibungskommentar für die Komponente gemäß Coderegeln.
- Verbesserung: Struktur der Datei beibehalten, aber besser lesbare Abschnitte geschaffen.
- Kleinere Optimierungen in der Lesbarkeit des Codes.
...
<template>
  <div class="form-control w-full custom-select">
    ...
    <div class="relative">
      ...
      >
        <span v-if="selectedDisplay" class="truncate">{{ selectedDisplay }}</span>
        <span v-else class="text-base-content/50">{{ placeholder || 'Auswählen...' }}</span>
        <span class="iconify" data-icon="mdi:chevron-down"></span>
      </div>

      <!-- Dropdown mit Optionen -->
      <div v-if="isOpen" class="absolute z-30 mt-1 w-full bg-base-100 rounded-box shadow-lg border border-base-300">
        <!-- Suchfeld -->
        <div class="p-2">
          <input
            type="text"
        ...
        <ul class="max-h-60 overflow-y-auto p-2">
          <li v-for="option in filteredOptions" :key="option.id" class="py-1">
            <label class="flex items-center space-x-2 cursor-pointer p-2 hover:bg-base-200 rounded-lg">
              <input
          ...
          <li v-if="filteredOptions.length === 0 && searchTerm" class="py-1">
            <div v-if="allowCreate" class="p-2 hover:bg-base-200 rounded-lg cursor-pointer" @click="createOption">
