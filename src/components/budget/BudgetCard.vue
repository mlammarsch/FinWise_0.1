...
const target = computed(() => {
  return props.targetAmount !== undefined ? props.targetAmount : props.category.targetAmount
})

// Verwende entweder den übergebenen aktuellen Betrag oder den Saldo der Kategorie
const current = computed(() => {
  return props.currentAmount !== undefined ? props.currentAmount : props.category.balance
})
    ...
    <div class="card-body p-4">
      <div class="flex justify-between items-center">
        <h3 class="card-title text-lg">
          {{ category.name }}
          <div v-if="!category.isActive" class="badge badge-sm badge-neutral ml-2">Inaktiv</div>
          <div v-if="category.isSavingsGoal" class="badge badge-sm badge-accent ml-2">Sparziel</div>
        </h3>
      ...
      <div class="mt-2">
        <p v-if="category.description" class="text-sm text-base-content/70 mb-2">{{ category.description }}</p>
