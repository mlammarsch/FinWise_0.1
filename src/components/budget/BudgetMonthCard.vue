...
import { Icon } from "@iconify/vue"; // Stellen Sie sicher, dass diese Zeile existiert
...
function openDropdown(event: MouseEvent, cat: Category) {
  ...
  debugLog("[BudgetMonthCard] Start openDropdown", {
    categoryId: cat.id,
  ...
  debugLog("[BudgetMonthCard] openDropdown complete", {
    categoryId: cat.id,
    categoryName: cat.name,
    x: dropdownX.value,
    y: dropdownY.value,
...
function optionFill() {
  ...
  const amountToFill =
    categoryData.saldo < 0 ? Math.abs(categoryData.saldo) : 0;
...
function executeTransfer(data: {
  ...
  // Im neuen Modus wird das Transfer-Handling direkt vom Modal übernommen
    ...
    <div class="flex-grow">
      ...
      >
        ...
        <div
          v-if="showDropdown"
          ref="dropdownRef"
          class="absolute z-40 w-48 bg-base-200 border border-base-300 rounded shadow-lg p-2"
